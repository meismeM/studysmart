...
import DashboardContainer from "@/components/DashboardContainer"; // Ensure correct import path
...
// Toaster is likely in layout.tsx, so not imported here unless specifically needed
// import { Toaster } from "@/components/ui/toaster";

// Import UserData type
import type { UserData } from '@/types/dashboard'; // Make sure path is correct
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (loggedInStatus === 'true' && storedUserData) {
      try {
        const parsedData: UserData = JSON.parse(storedUserData); // Type cast here
        setIsLoggedIn(true);
  ...
  // --- Login Handler ---
  const handleLoginSuccess = (loggedInUser: UserData) => { // Expects UserData type
      ...
      <div className="flex flex-col md:flex-row min-h-screen relative">
        ...
        <main className="w-full md:flex-1 p-6 md:p-10 xl:p-12 md:h-screen md:overflow-y-auto pb-28 md:pb-10">
          <DashboardContainer
            ...
            endPage={endPageNum}
            userId={userData?.id} // Pass the user ID here
          />
        </main>
