             ...
             {/* Notes Card */}
                    <CardContent className="flex-grow flex flex-col p-0">
                        {/* ... Loading/Empty states ... */}
                         { !generatedNotes.trim() && !isGeneratingNotes ? ( <div className="flex flex-col items-center justify-center flex-grow text-muted-foreground p-6 text-center"> <FileText size={48} className="mb-4 opacity-50" /> <p className="text-sm font-medium">No Notes Generated</p> <p className="text-xs mt-1">Click "Generate Notes" above or check input.</p> </div> )
                         : isGeneratingNotes ? ( <div className="flex-grow flex flex-col items-center justify-center text-muted-foreground p-6 text-center"> <Loader2 className="h-6 w-6 md:h-8 md:w-8 animate-spin mb-4 text-primary" /> <p className="text-xs md:text-sm font-medium">{noteGenerationMessage || "Generating notes..."}</p> </div> )
                         : (
                            <ScrollArea className="flex-grow w-full rounded-b-lg border-t dark:border-slate-700">
                                <div className="p-5 md:p-8">
                                    {/* ** PASS THE components PROP ** */}
                                    <ReactMarkdown
                                        className="prose prose-sm sm:prose-base dark:prose-invert max-w-none"
                                        remarkPlugins={[remarkGfm]}
                                        components={markdownComponents} // <-- USE THE CUSTOM COMPONENTS
                                    >
                                        {generatedNotes}
                                    </ReactMarkdown>
                                </div>
