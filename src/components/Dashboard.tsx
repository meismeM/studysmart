...
import ReactMarkdown from 'react-markdown'; // No need for 'Components' if not customizing individual elements
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  useEffect(() => {
    ...
    setGeneratedQuestions(initialQuestionState); setIsGeneratingQuestions(initialLoadingState); setQuestionGenerationMessage(initialMessageState); setShowAnswer(initialShowAnswerState);
    setComponentError(null); setActiveQuestionTab('multiple-choice');
                    ...
                    {isGeneratingNotes && !generatedNotes ? ( <div className="flex-grow flex flex-col items-center justify-center text-muted-foreground p-6 text-center"> <Loader2 className="h-6 w-6 md:h-8 md:w-8 animate-spin mb-4 text-primary" /> <p className="text-xs md:text-sm font-medium">{noteGenerationMessage || "Generating notes..."}</p> </div> )
                     ...
                     : (
                         <ScrollArea className="flex-grow w-full rounded-b-lg border-t dark:border-slate-700">
                             {/* This div contains the Markdown content and will have the padding.
                                 ReactMarkdown itself will expand to the width of its content if max-w-none is set.
                                 The ScrollArea will provide scrollbars if this div becomes wider than the ScrollArea.
                             */}
                             <div className="p-5 md:p-8">
                                <ReactMarkdown
                                    className="prose prose-sm sm:prose-base dark:prose-invert max-w-none"
                                    remarkPlugins={[remarkGfm]}
                                    // Removed components prop to let prose handle table/pre scrolling by default
                                >
                                    {generatedNotes}
                                </ReactMarkdown>
                             </div>
                             <ScrollBar orientation="horizontal" /> {/* Ensure this is present */}
                         </ScrollArea>
                     )}
                 </CardContent>
             </Card>
